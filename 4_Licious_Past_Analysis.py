...
# Set page title
st.title("Licious Price Analysis Over Time")

# Define the directory path for data files
DATA_DIR = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'licious_past')

def extract_datetime_from_filename(filename):
    # Extract date and time parts from filename (format: licious_city_2025_04_14_17_09_46.json)
...
def extract_city_from_filename(filename):
    # Extract city name from filename (format: licious_city_YYYY_MM_DD_HH_MM_SS.json)
    filename = os.path.basename(filename)
    parts = filename.split('_')
    # Find the first part that looks like a year (2025)
    year_index = next(i for i, part in enumerate(parts) if part.isdigit() and len(part) == 4)
    # City name is between 'licious' and the year
...
try:
    ...
    # Find all Licious JSON files in the specified directory
    files = glob.glob(os.path.join(DATA_DIR, 'licious_*_*.json'))
    
    if not files:
        st.error(f"No Licious data files found in the directory: {DATA_DIR}")
        st.info("Please ensure your data files are placed in the 'licious_past' directory and follow the naming convention: licious_city_YYYY_MM_DD_HH_MM_SS.json")
        st.stop()
    ...
    if len(sorted_dates) < 2:
        st.error(f"Insufficient data for {selected_city.title()}. Need at least 2 different dates for comparison.")
        st.stop()
    
    # Display date selection
    st.header(f"Select Analysis Period for {selected_city.title()}")
    ...
    st.info(f"Analyzing price changes in {selected_city.title()} from {start_datetime.strftime('%B %d, %Y at %I:%M %p')} to {end_datetime.strftime('%B %d, %Y at %I:%M %p')}")
    ...
    if price_changed_products.empty:
        st.warning(f"No products with price changes found in {selected_city.title()} for the selected period.")
    else:
        st.header(f"Price Trends for Changed Products in {selected_city.title()}")
        ...
        fig = px.line(
            ...
            title=f"Price Trend for {selected_product.split(' - ')[0]} in {selected_city.title()}",
            labels={
                'datetime': 'Date',
                'value': 'Price (₹)',
                'variable': 'Price Type'
        ...
        st.header(f"Summary Statistics for {selected_city.title()}")
        ...
        st.subheader(f"Category-wise Price Changes in {selected_city.title()}")
        ...
        if not filtered_df.empty:
            ...
            st.download_button(
                ...
                data=csv,
                file_name=f"licious_{selected_city}_price_changes_{start_datetime.strftime('%Y%m%d')}_to_{end_datetime.strftime('%Y%m%d')}.csv",
                mime="text/csv"
            )
